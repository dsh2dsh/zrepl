...
Writes all log entries with minimum level ``level`` to file from ``filename``.
If ``filename`` is not defined, logs everyting to ``stderr``. This outlet
automatically detects the log file was rotated and reopens it.
